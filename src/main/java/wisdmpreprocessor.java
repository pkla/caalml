...
public class wisdmpreprocessor extends setup {

    private static final Logger log = LoggerFactory.getLogger(wisdmpreprocessor.class);

    //'baseDir': Base directory for the data. Change this if you want to save the data somewhere else
    private static File baseDir = new File(dataDir);
    ...
    public static void main() throws Exception {
        ...
        // TODO add regexSequenceRecordReader to parse fordham's format
    ...
    private static void WISDM_ETL() throws Exception {
        ...
        List<Integer> aSegment = new ArrayList<>();
        List<Float> xSegment = new ArrayList<>();
        List<Float> ySegment = new ArrayList<>();
        List<Float> zSegment = new ArrayList<>();
        ...
        for (String line : lines) {
            ...
            if (line.contains("0,0,0") || s.length > 6 || s.length < 5) {
                ...
                try {
                    xSegment.add(Float.parseFloat(s[3]));
                    ySegment.add(Float.parseFloat(s[4]));
                    zSegment.add(Float.parseFloat(s[5]));
                    aSegment.add(map.get(s[1]));
                    lineCount++;
            ...
            if ((lineCount + 1) % 200 == 0) {
                ...
                Segment segment = new Segment(xSegment, ySegment, zSegment, aSegment);
                dataset.addSegment(segment);
                ...
                xSegment = new ArrayList<>();
                ySegment = new ArrayList<>();
                zSegment = new ArrayList<>();
            }

            // temp workaround for some unexplained behavior
            if (currSegment > 5490) {
                return;
            }
        }
        ...
        System.out.println("END WISDM_ETL");
        return;
    ...
    static class Segment {
        ...
        public Segment(List<Float> xSegment, List<Float> ySegment, List<Float> zSegment, List<Integer> aSegment) {
        ...
        public List<List<Float>> getFeatures () {
    ...
    static class DataSet {
        ...
        public int firstaSegmentSize() {
            return segments.get(0).aSegment.size();
        }
    }
}